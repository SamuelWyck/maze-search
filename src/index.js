...
const manager = new AlgorithmManger(20, 20);
...
display.setBoard(grid);
manager.setBoard(grid);
const [bfs, path] = manager.depthFirstSearch();
let index = 0;
function test() {
    index += 1;
    if (index >= bfs.length) {
        for (let pos of path) {
            const [row, col] = pos;
            display.setCell(row, col, "path");
        }
        return;
    }
    const [row, col] = bfs[index];
    display.setCell(row, col, "visited");
    setTimeout(test, 10);
};
